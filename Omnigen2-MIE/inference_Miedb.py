...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Begin of test sheet"
    )
    parser.add_argument(
        "--end",
        type=int,
        default=0,
        help="End of test sheet"
    )
    parser.add_argument(
        "--stride",
        type=int,
...
def main(args: argparse.Namespace, root_dir: str) -> None:
    ...
    args.benchmark_dir = "../dataBenchmark/"
    args.output_folder = "../inference/eval_results/Omnigen2"

    if args.transformer_lora_path:
        args.output_folder += '_mie_lora'
    elif args.transformer_path:
        args.output_folder += '_mie'
    else:
        args.output_folder += '_base'
    ...
    samples = samples[args.begin:args.end:args.stride]
    
    for sample in tqdm(samples, desc=f"Editing with OmniGen2 [{args.begin}:{args.end}:{args.stride}]"):
        image_path = os.path.normpath(os.path.join(args.benchmark_dir, sample['input_image']))
        input_images = preprocess(image_path)

        prompt =sample["prompt"]
        # Generate and save image
        output_path = os.path.join(args.output_folder, f"{sample['output_image'].split('/')[-1]}")
        if os.path.exists(output_path.replace(".png", f"_{args.num_images_per_prompt - 1}.png")):
            continue
        ...
        # output_image.save(os.path.join(args.output_path, f"{sample['id']}_0.png"))
        print(f"{sample['output_image'].split('/')[-1]} Saved")
