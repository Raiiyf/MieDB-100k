...
from DICE import extract_alpha, extract_alpha_multi_color
    ...
    for sample in tqdm.tqdm(data_sheet):
        ...
        for batch in range(NUM_BATCH):
            ...
            if missing_flag != 1:
                ...
                gt_mask[gt_mask != 0] = 1
                inverse_mask = (1 - gt_mask).astype(np.uint8)

                # Calculate PSNR only on the area outside the mask
                # total_score += compute_psnr(gt_image, inp_image, inverse_mask)
                gt_image[inverse_mask] = 255
                inp_image[inverse_mask] = 255
