...
def main():
    ...
    args = parse_args()
    test_sheet = test_sheet[args.begin_index::args.stride]

    pipeline = FluxKontextPipeline.from_pretrained(MODEL_PATH, torch_dtype=torch.bfloat16)
    print("pipeline loaded")
    pipeline.to('cuda')
    pipeline.set_progress_bar_config(disable=None)

    output_folder = os.path.join(OUTPUT_DIR, "FluxKontext")
    os.makedirs(output_folder, exist_ok=True)
    for sample in tqdm.tqdm(test_sheet): 
        if sample['category'] == "Modification":
            continue
        ...
        with torch.inference_mode():
            ...
            print(f"{idx} image saved at", os.path.abspath(f"{sample['output_image'].split('/')[-1]}"))
